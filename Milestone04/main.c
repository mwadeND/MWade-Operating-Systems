...
void * thread_producer (void * pData)
{
    ...
    while(KeepGoing)
    {
        // printf("Producer Thread %d - Iterations To Go: %d\n", ThreadID, IterationsToGo);
...
void * thread_consumer (void * pData)
{
    ...
    while(KeepGoing)
    {
        ...
        {
            KeepGoing = 0; // state for others to stop
            pthread_cond_broadcast(&NotEmptyCond); // tell all others that it is done
...
{    
    ...
    if(nThreadsProducers < 1 || nThreadsProducers > 20) {
        printf("Incorrect usage, number of producer threads should be an integer between 1 and 20\n");
        return -1;
    }
    if (nThreadsConsumers < 1 || nThreadsConsumers > 30) {
        printf("Incorrect usage, number of consumer threads should be an integer between 1 and 30\n");
        return -1;
    }
    if (nIterations < 1 || nIterations > 1000000) {
        printf("Incorrect usage, number of iterations should be an integer between 1 and 1,000,000\n");
        return -1;
    }
    ...
    printf("This program took %ld seconds\n", seconds);
